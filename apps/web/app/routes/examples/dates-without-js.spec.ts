...
testWithoutJS('With JS disabled', async ({ example }) => {
  ...
  await example.expectError(mandatory, 'Invalid input: expected date, received null')
  await example.expectValid(optional)
  await example.expectValid(nullable)
  await example.expectError(defaultField, 'Invalid input: expected date, received null')
